...
public class SideBar extends View {
    ...
    private int mChoose = -1;//当前选择的字母id

    private final float mDensity;
    private float mY;//当前移动触点的Y坐标
    private float mHalfWidth, mHalfHeight;//滚动条的X初始坐标 滚动条的Y高度
    private float mLetterHeight;//单个字母的高度
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < getLettersSize(); i++) {
        	//当前遍历字母i的最大Y值,应该优化为当前字母i的中点Y坐标
        	// + mLetterHeight/2 后手指的位置对了,但是最大字母不是高亮那个了。
            float letterPosY = mLetterHeight * (i + 1) + getPaddingTop();
            float diff;// diff:XY轴放大倍数
            float diffX;//diffX:使突出效果为半圆形，如果不加这两个参数是突出三角形。
            float diffY;//diffY:解决最大的那三个字母间距太近问题
...
//            System.out.println("X======="+(mHalfWidth * 1.20f + diffX));
//            System.out.println("Y======="+(letterPosY + diffY));
