...
public class BezierHeart extends View {
    ...
    private float mDuration = 1000;                     // 变化总数
    private float mCurrent = 0;                         // 当前已进行数
    private float nCurrent = 0;                         // 当前已进行数 反向变化(心-圆)
    private float mCount = 50;                         // 将时长总共划分多少份
    private float mPiece = mDuration/mCount;            // 每一份的数
    private float mDelay = 10;            				// 每一份的时长 似乎最小为10
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mCurrent < mDuration){
            ...
            postInvalidateDelayed((long) mDelay);
        ...
        if (nCurrent > 0){
            ...
            postInvalidateDelayed((long) mDelay);//相当于等待mDelay秒后再次刷新View调用onDraw()
        } else if(nCurrent == 0) {
        	mCurrent = 0;
        	nCurrent -= mPiece;
        	if(isCirculate) {
    ...
    public void reDraw() {
    	...
    	mData[1] = mCircleRadius;
        mCtrl[7] = -mCircleRadius;
        mCtrl[9] = -mCircleRadius;

        mCtrl[4] = mCircleRadius;
        mCtrl[10] = -mCircleRadius;
