...
public class LeafLoadingView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 改为叶子飘到进度条就消失，而不是飘到最左端。就不需要把叶子放到进度条下层了。
...
//        int leafWidth = 40;
//        leaf.x = (int) (mProgressWidth - (mProgressWidth-mCurrentProgressPosition) * fraction - leafWidth);
        leaf.y = getLocationY(leaf);
    }

    // 通过叶子信息获取当前叶子的Y值
    private int getLocationY(Leaf leaf) {
        // y = A*Sin(w(x+Q))+h  A影响振幅，w影响周期，Q横坐标位移（这里取0），h纵坐标位移。
